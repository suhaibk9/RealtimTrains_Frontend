...
import { setServiceData, setLocationsCoords } from '../utils/detailsSlice';
...
const ServiceDetails = () => {
 ...
 // const locationsCoords = useSelector((state) => state.details.locationCoords);
  ...
  useEffect(() => {
    if (serviceData) {
      const fetchCoordinates = async () => {
        ...
        // dispatch(setLocationsCoords(coords));
      ...
      <Container maxWidth="lg">
        ...
        >
          ...
          <Box sx={{ width: '100%', height: '500px', position: 'relative' }}>
            {locationsCoords &&
            locationsCoords.length > 0 &&
            locationsCoords[0]?.lat ? (
              <MapContainer
